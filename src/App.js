...
console.log(tasks);
  ...
  const onChangeHandler = function (event) {
    ...
    // `event.target.value` permet de récupérer la valeur d'un input
    // 👆 Il fonctionne qu'avec les balises formulaires (input, select)
    // event.target.classList.add('red') // ajoute la class `red` quand on entre une saisie
  ...
  const addTaskHandler = function (event) {
    ...
    // Stocke les taches dans le localstorage
  ...
  return (
    <main className="bg-slate-900 min-h-screen pt-5 px-10">
      ...
      <form
        ...
        <input
          // A chaque saisie sur le input la fonction est rééxecuter
          // Avec le paramètre `event` on peut accéder à l'élément `input`
      ...
      <section className="mt-10 md:w-2/3 mx-auto ">
        <ul className="flex flex-col space-y-5">
          {/* 
          La méthode `map` va parcourir la liste (tableau) et retourner (afficher) les éléments de la liste (tableau) dans le DOM
